...
public class AdminRegistrationFrame extends JFrame {
    ...
    public AdminRegistrationFrame() {
        ...
        gbc.gridx = 0;
        gbc.gridy++;
        formCard.add(UITheme.createLabel("Confirm Password *:"), gbc);
        gbc.gridx = 1;
        txtConfirmPassword = UITheme.createPasswordField(20);
        formCard.add(txtConfirmPassword, gbc);
        ...
        gbc.gridy++;
        lblValidation = UITheme.createLabel("* Required fields. Username: 4-30 letters, numbers, or underscores.", UITheme.SMALL_FONT, UITheme.TEXT_MUTED);
        formCard.add(lblValidation, gbc);
    ...
    private void registerAdmin() {
        try {
            ...
            if (username.isEmpty() || password.isEmpty() || confirmPassword.isEmpty()) {
                showValidation("All required fields must be completed.");
                return;
            }
            if (!com.mu.util.InputValidator.isValidUsername(username)) {
                showValidation("Username must be 4-30 letters, numbers, or underscores.");
                return;
            }
            if (!password.equals(confirmPassword)) {
                showValidation("Password and confirmation do not match.");
                return;
            }
    ...
    private void showValidation(String message) {
        lblValidation.setForeground(UITheme.DANGER_COLOR);
