...
public class RegisterFrame extends JFrame {
    ...
    public RegisterFrame() {
        ...
        header.add(UITheme.createLabel("Fields marked * are required", UITheme.BODY_FONT, UITheme.TEXT_MUTED));
        content.add(header, BorderLayout.NORTH);
        JPanel card = UITheme.createCardPanel();
        card.setLayout(new GridBagLayout());
        GridBagConstraints g = c();
        label(card, g, "Student Registration", UITheme.SUBHEADER_FONT);
        g.gridy++;
        ...
        JButton register = UITheme.createPrimaryButton("Register");
        JButton back = UITheme.createOutlineButton("Back");
        actions.add(register);
        actions.add(back);
        card.add(actions, g);
        g.gridy++;
        label(card, g, "Already have an account? Login instead.", UITheme.SMALL_FONT);
    ...
    private JTextField field(JPanel p, GridBagConstraints g, String label, String hint) {
        g.gridwidth = 1;
        label(p, g, label, UITheme.BODY_BOLD);
        g.gridx = 1;
        JTextField field = UITheme.createTextField(20);
    ...
    private void registerStudent() {
        ...
        if (txtId.getText().trim().isEmpty() || txtName.getText().trim().isEmpty() || txtDepartment.getText().trim().isEmpty() || txtBatch.getText().trim().isEmpty() || txtPhone.getText().trim().isEmpty()) {
            FormSupport.showError(lblError, "Complete every required field.");
            return;
        }
        if (!InputValidator.isValidEmail(txtEmail.getText().trim())) {
        ...
        try {
            ...
            s.setDepartment(txtDepartment.getText().trim());
            s.setBatch(txtBatch.getText().trim());
            s.setPhone(txtPhone.getText().trim());
            s.setPassword(password);
            registrationService.register(s);
