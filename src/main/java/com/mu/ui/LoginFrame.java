...
public class LoginFrame extends JFrame {
    ...
    public LoginFrame( ) {
        ...
        add( card, g, "Student Login", UITheme.SUBHEADER_FONT );
        g.gridy++;
        add( card, g, "Email Address *", UITheme.BODY_BOLD );
        g.gridx = 1;
        txtEmail = UITheme.createTextField( 20 );
        txtEmail.setToolTipText( "name@example.com" );
        card.add( txtEmail, g );
        g.gridx = 0;
        g.gridy++;
        g.gridwidth = 2;
        add( card, g, "Use your university email address.", UITheme.SMALL_FONT );
        g.gridy++;
        g.gridwidth = 1;
        add( card, g, "Password *", UITheme.BODY_BOLD );
        g.gridx = 1;
        ...
        g.gridy++;
        g.gridwidth = 2;
        card.add( FormSupport.createShowPasswordToggle( txtPassword ), g );
        g.gridy++;
        ...
        JButton login = UITheme.createPrimaryButton( "Login" );
        JButton register = UITheme.createOutlineButton( "Register" );
        buttons.add( login );
        buttons.add( register );
        card.add( buttons, g );
        g.gridy++;
        JPanel admin = new JPanel( new GridLayout( 1, 2, 12, 0 ) );
        admin.setOpaque( false );
        JButton adminLogin = UITheme.createSecondaryButton( "Admin Login" );
        JButton adminRegister = UITheme.createSecondaryButton( "Create Admin Account" );
        admin.add( adminLogin );
        admin.add( adminRegister );
        card.add( admin, g );
        g.gridy++;
    ...
    private void add( JPanel p, GridBagConstraints g, String text, Font font ) {
        p.add( UITheme.createLabel( text, font, font == UITheme.SMALL_FONT ? UITheme.TEXT_MUTED : UITheme.TEXT_DARK ), g );
    }
    ...
    private void login( ) {
        ...
        if ( !InputValidator.isValidEmail( email ) ) {
            FormSupport.showError( lblError, "Enter a valid email address." );
            return;
        }
        if ( password.isEmpty( ) ) {
            FormSupport.showError( lblError, "Password is required." );
            return;
        }
        try {
            ...
            if ( student != null ) {
                ...
                new DashboardFrame( );
                dispose( );
            } else FormSupport.showError( lblError, "Invalid email or password." );
        } catch ( Exception ex ) {
